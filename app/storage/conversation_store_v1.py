...
class ConversationStoreV1:
        ...
        with self._connect() as db:
            db.execute(
                ...
                INSERT INTO conversations(
                    conversation_id,user_id,birth_profile_id,title,life_context_json,created_at,updated_at
                ) VALUES(?,?,?,?,?,?,?)
                """,
                (
                    conversation_id,
    ...
    def update_conversation(
        ...
        *,
        ...
        with self._connect() as db:
            db.execute(
                f"UPDATE conversations SET {', '.join(assignments)} WHERE user_id=? AND conversation_id=?",
    ...
    def add_message(
        ...
        *,
        ...
        route: str | None = None,
        reference_moment: str | None = None,
        ...
        with self._connect() as db:
            db.execute(
                ...
                INSERT INTO conversation_messages(
                    message_id,conversation_id,user_id,role,content,domain,route,reference_moment,payload_json,created_at
                ) VALUES(?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    message_id,
            ...
            row = db.execute(
                "SELECT * FROM conversation_messages WHERE message_id=? AND user_id=?",
                (message_id, user_id),
            ).fetchone()
        if row is None:
