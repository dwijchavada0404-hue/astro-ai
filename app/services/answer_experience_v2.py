...
def present_answer_v2(routed: dict[str, Any], language: AnswerLanguage = "hinglish") -> str | None:
    ...
    if domain == "friends_social_community":
        synthesis = _dict(result.get("synthesis"))
        event_result = _dict(result.get("event_result"))
        future = _range(_timing_period(result, "future")) or _range(synthesis.get("strongest_future_period"))
        focus = event_result.get("label") or synthesis.get("strongest_area")
        if isinstance(focus, str) or future:
            focus_text = str(focus or "friendship and social connection").replace("_", " ")
    ...
    if domain == "life_settlement":
        synthesis = _dict(result.get("synthesis")); timing = _dict(result.get("timing"))
        future = _range(timing.get("strongest_convergence_window"))
        foundations = synthesis.get("strongest_domains")
        foundation_text = ", ".join(str(item).replace("_", " ") for item in foundations) if isinstance(foundations, list) and foundations else "your core life areas"
        if future:
            return {"hinglish":f"Aapke chart mein {future} ke aas-paas career, relationship, home ya finances jaise life areas mein combined support comparatively zyada dikh raha hai. Isse life 'settle' hone ki fixed deadline na samjhein; aapki choices aur actual circumstances hi in areas ko shape karte hain.","english":f"Your chart shows comparatively more combined support across life areas such as career, relationships, home or finances around {future}. Do not treat this as a fixed deadline by which life will be 'settled'; your choices and real circumstances are what shape these areas.","hindi":f"आपकी कुंडली में {future} के आसपास करियर, रिश्ते, घर या वित्त जैसे जीवन के क्षेत्रों में संयुक्त सहयोग तुलनात्मक रूप से अधिक दिखाई देता है। इसे जीवन के 'सेटल' होने की निश्चित समय-सीमा न मानें; आपकी पसंद और वास्तविक परिस्थितियाँ ही इन क्षेत्रों को आकार देती हैं।"}[language]
        if foundation_text:
