...
def route_family_children_question_v1(chart: dict[str, Any], question: str, reference_moment: datetime) -> dict[str, Any]:
    ...
    if not understanding.get("available"):
        return {"available": False, "route": "unsupported", "event": "unknown", "understanding": understanding, "reason": "The question was not identified as a Family & Children question."}
    ...
    if intent in EVENT_MAP:
        ...
        return {"available": bool(events.get("available")), "route": "family_children_event_v1", "event": "family_children", "event_key": event_key, "primary_intent": intent, "understanding": understanding, "event_intelligence": events, "event_result": event_result, "answer": events.get("answer") if events.get("available") else events.get("reason"), "limitation": events.get("limitation") or natal.get("limitation"), "children_question_boundary": events.get("children_question_boundary")}

    if intent == "family_direction":
        direction = analyze_family_children_direction_v1(chart)
        return {"available": bool(direction.get("available")), "route": "family_children_direction_v1", "event": "family_children", "primary_intent": intent, "understanding": understanding, "direction": direction, "answer": direction.get("answer") if direction.get("available") else direction.get("reason"), "limitation": direction.get("limitation") or natal.get("limitation")}

    return {"available": bool(natal.get("available")), "route": "family_children_natal_v1", "event": "family_children", "primary_intent": intent, "understanding": understanding, "natal": natal, "answer": natal.get("summary") if natal.get("available") else natal.get("reason"), "limitation": natal.get("limitation")}
