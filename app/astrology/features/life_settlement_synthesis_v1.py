...
DOMAIN_ORDER = ("career", "finance", "marriage", "property_home", "family_children", "location_settlement")
DOMAIN_LABELS = {
    ...
    "location_settlement": "Location & Foreign Settlement",
}
...
def _extract_domain_score(domain: str, result: dict[str, Any]) -> float:
    if domain == "location_settlement":
        ...
        return _bounded(max(
            _safe_float(scores.get("rooted_home_base")),
            _safe_float(scores.get("long_distance_residence")),
            _safe_float(scores.get("foreign_settlement")),
        ))
    keys = {
...
def _collect_domain(name: str, fn: Callable[..., dict[str, Any]], chart: dict[str, Any], reference_moment: datetime) -> tuple[dict[str, Any], dict[str, str] | None]:
    try:
        ...
        }, {"domain": name, "error_type": type(exc).__name__, "message": str(exc)}


def analyze_life_settlement_synthesis_v1(chart: dict[str, Any], reference_moment: datetime) -> dict[str, Any]:
    """Orchestrate mature domains into one cross-domain life synthesis."""
    ...
    engines: dict[str, Callable[..., dict[str, Any]]] = {
        ...
        "location_settlement": analyze_location_settlement_synthesis_v1,
    }
    ...
    if not available_domains:
        return {"available": False, "event": "life_settlement_synthesis", "model_version": "v1", "reason": "No mature life-domain synthesis was available.", "components": components, "collection_errors": collection_errors}

    weighted = [domain_scores[d] * max(0.35, domain_confidence[d]) for d in available_domains]
    weights = [max(0.35, domain_confidence[d]) for d in available_domains]
    settlement_score = _bounded(sum(weighted) / sum(weights)) if weights else 0.0
    coverage = len(available_domains) / len(DOMAIN_ORDER)
    confidence = _bounded(0.34 + 0.30 * coverage + 0.26 * (sum(domain_confidence.values()) / len(available_domains)))
    outlook = "broadly_supportive" if settlement_score >= 0.70 else "moderately_supportive_with_mixed_areas" if settlement_score >= 0.50 else "mixed_or_developmental"
    ...
    for domain in available_domains:
        ...
        current_signals[domain] = result.get("active_present_period") or result.get("current_timing_outlook") or result.get("near_term_direction")
        future_signals[domain] = result.get("strongest_future_period") or result.get("strongest_future_window") or result.get("strongest_future_event")

    strongest_labels = [DOMAIN_LABELS[name] for name in strongest_domains]
    summary = f"Cross-domain symbolic settlement is {outlook.replace('_', ' ')}. The strongest current foundations are {', '.join(strongest_labels)}."
    if development_domains:
        summary += " Areas that may need more deliberate real-world development are " + ", ".join(DOMAIN_LABELS[name] for name in development_domains) + "."
    ...
    return {
        ...
        "historical_validation": {
            ...
            "rule": "Known real-world career, financial, relationship, property, family, residence and migration history overrides astrological inference. Past symbolic windows may only be used to interpret milestones the user has confirmed.",
        },
        "answer": summary,
        "limitation": (
            "This is a cross-domain symbolic astrology synthesis, not a deterministic life forecast. It does not guarantee career success, wealth, marriage, property ownership, fertility, pregnancy, childbirth, family outcomes, relocation, immigration, citizenship, foreign settlement or a specific age/date of being 'settled'. Real-world facts and decisions take priority over astrological inference."
        ),
    }
