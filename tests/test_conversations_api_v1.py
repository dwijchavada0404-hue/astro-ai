...
def test_conversation_crud_is_authenticated_and_owned(tmp_path):
    ...
    assert client.get("/api/v1/conversations").json()["conversations"][0]["conversation_id"] == conversation_id
    assert client.patch(f"/api/v1/conversations/{conversation_id}", json={"title": "Renamed"}).json()["conversation"]["title"] == "Renamed"
